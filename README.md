...
| ④ 演示真实性 | 引入 **9/16 美联储加息 25bp 落地**（3.50%–3.75% → 3.75%–4.00%）作为**最高优先级置顶资讯**，全市场数据（美债/美元/人民币/黄金/美股）同步重算以保持逻辑自洽 |
...
| 评审意见 | 落地内容 |
|---|---|
| ① 简报逻辑过度依赖美国 | **信源权重重构为「国内 60% / 海外 40%」**：资讯池固定为 **国内 9 条 + 海外 6 条 = 15 条**，并做**真实国内时政选材**（央行 MLF 净投放、LPR 连续 15 个月不变、下半年"适度宽松"定调、工信部"人工智能+软件"专项行动、国务院《扩大消费"十五五"规划》批复、中长期资金入市"长钱长投"、北向资金）；简报首页即标注**信息来源权重：国内 60%（9 条）· 海外 40%（6 条）**，宏观变量条**国内优先排列**并带 🇨🇳/🌎 标记 |
| ② 客户信息替换为真实数据 | 依据 **`客户数据.xls`（6 位真实客户 / 25 笔真实持仓）** 重建客户库：**安女士 / 刘先生 / 任先生 / 谢先生 / 钱女士 / 高先生**，持仓标的、**真实盈亏率**、仓位、风险等级（C3–C5）、换手率、持仓风格、有无买过产品全部还原；**可编辑功能完整保留**（客户档案可编辑姓名/风险等级/资产规模/换手率/风格/买过产品，持仓可编辑盈亏率/仓位） |
...
> **置顶项变更**：V3 起，最高优先级置顶资讯由「美联储加息」改为**国内「央行 MLF 净投放 1200 亿 + 下半年延续适度宽松定调」（6 渠道交叉验证）**，体现"国内定价主导"的简报逻辑。
...
- **隔夜美股** —— 道指 / 标普 / 纳指 / 费半 四张卡片
- **A 股昨日收盘** —— 上证指数 / 深证成指 / **创业板指** / 科创 50，含成交额（新增）
- **关键变量** —— **国内优先**：1年期 LPR / 5年期以上 LPR / DR007 / MLF 净投放 / 北向资金；海外：美元指数 / 10Y 美债 / 离岸人民币 / 原油 / 黄金
...
[08:00:03] 📰 资讯：拉取财联社 / 东财 Choice / 东财网 全网要闻 78 条 ✅
...
|---|---|---|---|---|---|
| 安女士 | 215 万 | 低 | 私募 | C5 进取型 | 2 只（均为私募基金） |
| 刘先生 | 232 万 | 低 | 大市值老登股 | C4 成长型 | 4 只（4 股票） |
| 任先生 | 501 万 | 高 | 股债 | C4 成长型 | 4 只（3 股票 + 标准券） |
| 谢先生 | 133 万 | 高 | 科技 | C4 成长型 | 12 只（高集中度科技股） |
| 钱女士 | 30 万 | 中 | 基金和债 | C4 成长型 | 2 只（指数增强基金 + 标准券） |
| 高先生 | 23 万 | 中 | 股票集中度高 | C3 平衡型 | 1 只（单一标的 53.6%） |

**持仓可编辑（保留）**：

- 客户工具栏支持「编辑档案」（姓名 / 风险等级 / 资产规模 / 换手率 / 持仓风格 / 有无买过产品 / 备注）与「新增客户」
- 持仓栏支持「新增持仓」（手填代码 / 名称 / 类型 / 行业 / 权重 / **盈亏率** / 风险分）与悬停编辑 / 删除
- **真实盈亏率**驱动持仓卡与组合诊断（谢先生含 *ST闻泰、退市标的联络3 等真实风险样本）
...
高先生 C3 平衡型 → 可售 R1-R3（超 R3 产品自动隐藏）
刘先生 / 任先生 / 谢先生 / 钱女士 C4 成长型 → 可售 R1-R4
安女士 C5 进取型 → 可售 R1-R5（全产品池）
```
...
├── data.js             # Mock 数据层（行情/资讯/QA/6 位真实客户/22 只产品库/敏感词库/文案模板）
├── app.js              # 交互剧本引擎 + 客户/持仓 CRUD + 产品匹配引擎 + 三方融合文案生成
├── style.css           # 深色金融终端风格样式
├── assets/             # 截图资源
